...
class MacScreenCapture:
        ...
        while self.running:
            try:
                ...
                if (
                    ...
                    and (datetime.now() - self.last_capture).seconds >= self.periodic_interval
                ):
    ...
    async def update_window_info(self, window_info: Dict):
        ...
        if window_info != self.current_window:
            ...
            if (
                ...
                and (datetime.now() - self.window_start_time).seconds >= self.min_window_duration
            ):
                await self.smart_capture()
    ...
    async def smart_capture(self):
        ...
        if window_key not in self.capture_history and self.daily_captures < self.max_daily_captures:
            screenshot_path = await self.capture(
                window=True,
                filename=self._generate_filename(app_name, window_title)
            )
    ...
    async def capture(self,
                     ...
                     no_shadow: bool = False) -> Optional[Path]:
        """
    ...
    def _generate_filename(self, app_name: str, window_title: str) -> str:
        ...
        window_title = "".join(c for c in window_title[:50] if c.isalnum() or c in (' ', '-', '_'))
